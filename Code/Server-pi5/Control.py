...
class Control:
    def __init__(self):
        ...
        self.order = ['', '', '', '', '']
        self.point: List[List[Union[int, float]]] = [[0, 99, 10], [0, 99, 10], [0, 99, -10], [0, 99, -10]]
        ...
        self.Thread_condition.daemon = True  # Allows program to exit even if thread is running
    ...
    def checkPoint(self):
        ...
        for i in range(4):
            leg_length[i] = math.sqrt(self.point[i][0] ** 2 + self.point[i][1] ** 2 + self.point[i][2] ** 2)
    ...
    def condition(self):
        ...
        while not self.stop_condition_thread:
            try:
                ...
                if current_time - self.move_timeout > 60 and self.move_timeout != 0 and self.relax_flag:
                    with self.lock:
                        self.move_count = 0
                ...
                if self.move_count < 180:
                    if (current_time - self.timeout) > 10 and self.timeout != 0 and not self.relax_flag and self.order[0] == '':
                        self.timeout = current_time
                        self.relax_flag = True
                        self.relax(True)
                        self.order = ['', '', '', '', '']
                    if self.relax_flag and self.order[0] != '' and self.order[0] != cmd.CMD_RELAX:
                        self.relax(False)
                        self.relax_flag = False
                    if self.attitude_flag and self.order[0] != cmd.CMD_ATTITUDE and self.order[0] != '':
                        self.stop()
                        self.attitude_flag = False
    ...
    def changeCoordinates(self, move_order, X1: Union[int, float]=0, Y1: Union[int, float]=96, Z1: Union[int, float]=0, X2: Union[int, float]=0, Y2: Union[int, float]=96, Z2: Union[int, float]=0, pos=np.mat(np.zeros((3, 4)))):
    ...
    def IMU6050(self):
        ...
        while not self.stop_condition_thread:
            ...
            self.changeCoordinates('Attitude Angle', pos=pos)
            if (self.order[0] == cmd.CMD_BALANCE and self.order[1] == '0') or (self.balance_flag and self.order[0] != '') or (self.move_count > 180):
